...
function trial_data = split_per_trial(raw_data)
    ...
    for counter = 1:length(header_start)
        ...
        indices = find(raw_data(curr_index + 1:header_stop - 1, 2) == 1 & raw_data(curr_index + 1:header_stop - 1, 3));
        values = raw_data(curr_index + indices, 3);
        if length(values) ~= 2
            error('Failed to retrieve experiment type and/or condition number!');
        end
        ...
        % Adding a new trial.
        trial_data(counter).start        = raw_data(curr_index, 1);
        trial_data(counter).header_stop  = raw_data(header_stop, 1);
        trial_data(counter).stop         = raw_data(trial_stop, 1); 
        trial_data(counter).exp_type     = values(1);
        trial_data(counter).condition    = values(2);
        trial_data(counter).photocell    = photocell;
        trial_data(counter).displacement = displacement;
    ...
    for counter = 1:length(trial_data)
        stim_dur(end + 1) = trial_data(counter).photocell(2) - trial_data(counter).photocell(1);
    end
    
    figure;
    plot(1000.0 * stim_dur), xlabel('trial no.'), ylabel('stimulus presentation, msec');
