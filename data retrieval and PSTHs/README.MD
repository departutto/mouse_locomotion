...
<br><br>
<b>Step 4.</b> Combine the locomotion data together with stimulus presentations. For that you need to have one .mat file that contains variable 'trials' (e.g., 2018-05-08-mouse-68865-run-01.mat) and one .m file with the locomotion data split per stimulus presentation (e.g., locomotion.m). You need to run the Matlab script <a href='https://github.com/departutto/mouse_locomotion/blob/master/data%20retrieval%20and%20PSTHs/incorporate_speed_signal.m'>'incorporate_speed_signal.m'</a> and provide the two aforementioned files as input to the script. The script will extend variable 'trials' stored in the .mat file by adding field 'velocity' that contains the locomotion signal per stimulus presentation.<br>
<b>Note!</b> By default the script <a href='https://github.com/departutto/mouse_locomotion/blob/master/data%20retrieval%20and%20PSTHs/incorporate_speed_signal.m'>'incorporate_speed_signal.m'</a> assumes that each trial is made up of two separate stimulus presentations. If this is not the case, you should specify the number of stimulus presentation per trial and provide it in the input to the script.
